...
## 2026-09-24 深夜：标题规则收紧 + 四个图表

### 标题规则（最终，全在 `pipeline/src/sources/paperr/normalize.ts`）

**news 现在也带原名了**：`newsN (原名)`，和 `unknownN (原名)` 一致。
41 → 隐藏 5 → **36 行**：**28 news** + 7 unknown + **1 原样**（`现代汉英词典_B00771V9HS`）。

新增的第四条规则 `headline`：**长标题（≥40 字）且作者不是真人**。
真实导出里 4 条裸标题（Samsung / Amanda Wakeley / 巴西牛肉 / Our World in Data）
就是这样抓到的 —— 它们的作者是 `N/A` 或 `EpubPressX`，和它们被打开的摘要源同一个。

⚠️ `NOT_AN_AUTHOR` 这个集合比标题更可靠：`N/A`、`EpubPressX`、`RSS Daily Digest`
都不是作者。**4 条裸标题和 12 条 EpubPressX 摘要共用同一个作者值**，靠标题区分不了。

⚠️ 长度是启发式，当成启发式对待：40 字远高于导出里最长的真标题（词典 17 字），
远低于最短的标题（51 字）。真被误判也不丢数据 —— 原名在括号里，原始导出没动。

### 页面：四个图表 + 交互

`components/PaperrCharts.tsx` —— 圆饼 / 平滑曲线 / 柱状 / 热力图。全部 SVG。
交互：**点图例选中类别**（圆心从总量变成占比）、**拖曲线看每天**、
**点柱子看星期几**、**点格子看那天**。

⚠️ 曲线要监听 DOM 而不是 Taro 的 `onMouseMove` —— Taro 的事件表里没有可靠的
`mousemove`，绑上去会被静默丢弃。抄 `MonthlyWave` 的做法。

⚠️ 数据天花板：payload 里**没有按小时、也没有按书按天**的数据，
所以「什么时段读的」这种图**这份数据支持不了**。四个图是日序列真能回答的问题。

### ⚠️⚠️ 又一次：70 条断言全过，页面还是错的

四个只有**看截图**才能发现的 bug：

| bug | 症状 | 原因 |
|---|---|---|
| 图例数值没格式化 | `44559 分钟` | `unit` 一个 prop 干了两种活：圆心单位 + 图例后缀 |
| **图例类别名被挤没** | 三个点 + 三个数字，不知道哪个是哪个 | 长数值抢走整行，`.pc-legend__label` 被压到 0 宽 |
| 统计数字换行 | `周一` 渲染成孤零零一个「周」 | 统计值是 40px 大字，两个汉字放不下 |
| 柱子标签换行 | `3.7 小` / `时` | 柱宽约 40px，`mins()` 输出带单位词 |

修完 70/70 仍全过。**断言看不见排版 —— 第三次了。**
...
## 2026-09-24 收尾（二）：公开/私有分界 + 已发布

### 分界（用户澄清后）

一开始理解偏了 —— 把整个阅读历史都挪到鉴权后面。用户澄清：**阅读记录可以完全公开，
只有「目前在读」需要口令。**
...
看得见，而不是靠某段代码记得剥掉一个字段。`dataVersion` 按公开 body 重算：
复用原来那个会把公开文件里根本不存在的 `current` 也算进去。

### ⚠️⚠️ 差点把私有文件发布出去

`deploy-pages.sh` 用的是 `cp -R data` —— **`.gitignore` 保护的是 `main`，
对这个脚本一点用都没有**，它是从文件系统建站的。
不改的话 `current.json`（你现在在读哪本）会出现在人人可读的站点上，
在一个可猜的 URL 后面，而且**无声无息**。

已加：显式排除清单 + 删完**复查**，没删掉就中止发布。发布日志里现在能看到
`▸ 已排除私有路径 …` 三行。

⚠️ **以后任何 gitignore 掉的东西，必须在同一个提交里加进 `PRIVATE` 数组。**

### 顺带把 GitHub 依赖整个去掉了

数据不提交 ⇒ 不需要 PAT、不需要 Actions 权限、不需要 workflow 文件。
`deploy.sh` 现在只要两个环境变量。

### 已验证

- ingest **60/60**（含「current 端点不携带书架」——防止它变成读整库的第二个入口）
- 页面 **80/80**（含最重要那条：**锁住一个面板不能锁住整个仪表盘**）
- 已推送 `main`，已发布 `gh-pages`

### 仍未做

- **阿里云部署**（安全组 8789 + 一条命令），见 `GO-LIVE.md`
- 线上页面的「在读」要等服务器起来才有数据；其余部分现在就是活的
