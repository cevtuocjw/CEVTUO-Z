...
插件挂了四个钩子，都在后台静默执行（不弹窗）：

| 时机 | 延迟 | 说明 |
|---|---|---|
| **WiFi 连上** | 3 秒 | 最常见的情况。飞机落地、回到家、开热点 |
| **KOReader 启动** | 12 秒 | ⚠️ 补漏用的，见下面 |
| **合上一本书** | 1 秒 | 一直在网时，读完一章就知道 |
| **设备休眠前** | 8 秒 | 一直在网、直接合盖时兜底 |

⚠️ **为什么需要「启动」这一条。** 2026-09-24 实际发生过一次：装了新插件、重启
KOReader、WiFi 一直开着，**十七分钟什么都没推**。那三个钩子全都看不见 ——
WiFi 从没断过所以 `NetworkConnected` 不触发，一本书都还没合上，设备也还没休眠。
表现和「插件坏了」一模一样。
